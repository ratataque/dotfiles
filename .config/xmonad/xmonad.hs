...
myFont :: String
myFont = "xft:SauceCodePro Nerd Font Mono:regular:size=19:antialias=true:hinting=true"
...
myWorkspaces = [" todo ", " dev ", " www ", " sys ", " doc ", " vbox ", " later ", " vid ", " chat "]
...
showKeybindings x = addName "Show Keybindings" $ io $ do
  h <- spawnPipe $ "yad --text-info --fontname=\"SauceCodePro Nerd Font Mono 12\" --fore=#19A094 back=#282c36 --center --geometry=1200x800 --title \"XMonad keybindings\""
...
myKeys c =
  ...
  , ("M-C-r", addName "Run prompt"      $ sequence_ [spawn (mySoundPlayer ++ dmenuSound), spawn "~/.local/bin/dm-run"])
  ...
  [ ("M-r", addName "Launch terminal"   $ spawn (myTerminal))
  ...
  ^++^ subKeys "lock screen"
  [ ("M-C-S-l", addName "lock screem"    $ spawn "xfce4-screensaver-command --lock") ]
  ...
  xmonad $ addDescrKeys' ((mod4Mask, xK_F1), showKeybindings) myKeys $ ewmh $ ewmhFullscreen $ docks $ def -- Just remove $ ewmhFullscreen if YouTube fullscreen doesn't work
    ...
    , logHook = dynamicLogWithPP $  filterOutWsPP [scratchpadWorkspaceTag] $ xmobarPP
        ...
        , ppVisible = xmobarColor color04 "" . clickable
          -- Hidden workspace
        , ppHidden = xmobarColor color11 "" . wrap
                     ("<box type=Top width=2 mt=2 color=" ++ color11 ++ ">") "</box>" . clickable
          -- Hidden workspaces (no windows)
        , ppHiddenNoWindows = xmobarColor color11 ""  . clickable
